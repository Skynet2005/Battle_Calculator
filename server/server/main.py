"""Shim so tests can import `server.main` when the top-level `server` package is on sys.path.
This file mirrors ../main.py; keep updates in sync.
"""
# server/main.py
...
def analyze(req: AnalysisRequest):
    ...
    try:
        ...
        if atk_sum and def_sum:
            lines.append(
                "Survivorship and losses suggest the momentum:"  # statement intro
            )
            lines.append(
        ...
        # Passive and hero-skill impacts
        passive = res.get("passive_effects") or {}
        proc_stats = (res.get("proc_stats") or {})
        highlights: List[str] = []
        # Pull a few most frequent hero skill procs per side
        def top_procs(side_key: str) -> List[str]:
        ...
        if atk_top:
            highlights.append("Attacker skill highlights: " + ", ".join(atk_top) + ".")
        if def_top:
            highlights.append("Defender skill highlights: " + ", ".join(def_top) + ".")

        if highlights:
            lines.extend(highlights)

        # Troop composition hint
        ...
        def fmt_ratios(rs):
            ...
            return ", ".join(parts)
        if atk_ratios:
            lines.append("Attacker ratios — " + fmt_ratios(atk_ratios) + ".")
        if def_ratios:
            lines.append("Defender ratios — " + fmt_ratios(def_ratios) + ".")
        ...
        # Build counter-strategy for the side that lost (heuristic fallback)
        ...
        # Baseline recommended ratios from mechanics
        ...
        # Heuristics
        # 1) If low damage (kill_pct < 0.45), shift toward DPS
        if loser_sum and loser_sum.get("kill_pct", 0) < 0.45:
            if loser == "defender":
                # Push some Lancers for bypass and keep a small Marksmen presence
                rec = {"Infantry": 0.55, "Lancer": 0.35, "Marksman": 0.10}
            else:
                rec = {"Infantry": 0.30, "Lancer": 0.30, "Marksman": 0.40}

        # 2) If front collapsed (very low Infantry survivors), raise Infantry share
        try:
